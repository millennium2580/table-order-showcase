    ...
    mainTabBtn.forEach(function(btn){
        btn.addEventListener("click", function(){
            ...
            const mainFiler = menus.filter(function(item){
                ...
                // // 1. best는 판매량 기준으로 sort할 예정이라 모두 통과시킬 것
                    // if(mainName == "best") return true;  // 모든 자료를 넘김
...
// #region — 🅂🅄🄱 카테고리를 화면에 그리는 함수 —————————— 
    function renderSubCate(arr, mainName){
        ...
        let html = "";
        arr.forEach(function(sub,inx){ //서브 생성
            let subClass = "";
            if(inx == 0) subClass = "sub-on"; // 클릭한 메인의 첫번째 서브만 스타일 활성화
            html += `<li data-sub="${sub}" class="${subClass}"><button class="caption-subInfo">${sub}</button></li>`;
        
        });

        // console.log(html);
        subTabArea.innerHTML = html; // 서브를 화면에 그리기 완료 
...
// #region — 선택한 상품의 옵션 영역을 렌더링 하는 함수 —————————— 
    ...
    function renderProductOption(choiceProduct){
        ...
        baseHtml += `<p class = "caption-subInfo">${choiceProduct.acntTaste}</p>`;
        baseHtml += `<p class = "caption-subInfo">알레르기 정보: ${choiceProduct.acntAllergy}</p>`;
...
// #region — 옵션 버튼 클릭하면 할 일 —————————— 
    ...
    optionList.addEventListener("click",function(e){
        ...
        // 김기사 strong을 포함한 button일 때만 이벤트 발생훼~~
    ...
    cartArr.forEach(function(item, inx){ // inx(carArr 일치하는 상품 삭제)
        ...
        if(item.options){
            for(let key in item.options){
                if(optionBtnName[key]){ // 옵션키가 있을때만 매핑
                    // console.log("장바구니-옵션키값", optionBtnName[key][item.options[key]]);
                    label = optionBtnName[key][item.options[key]];
                }else{
            ...
            <li class="item" data-index = "${inx}">
                ...
                <div class = "cart-info">
                    ...
                    <div class="quantity">
                        ...
                        <button class="remove" data-btn="remove">삭제</button>
                    </div>
                </div>
            </li>
        `;
...
function renderCartTotal(){
    ...
    order_El.textContent = orderPrice.toLocaleString();
}
// #endregion
